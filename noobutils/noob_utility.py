...
from .noob_converters import NoobCoordinate
from .noob_exceptions import ButtonColourNotFound, MemberOrGuildNotFound

def is_have_avatar(thing: Union[discord.Member, discord.Guild] = None, display_av = False) -> str:
    if thing is None:
        return ""
        ...
        raise MemberOrGuildNotFound(
            f'Member or Guild "{thing}" was not found.'
        )
...
def get_button_colour(colour: str) -> discord.ButtonStyle:
    ...
    if colour.lower() == "blurple":
        ...
        raise ButtonColourNotFound(
            f'"{colour}" is not a valid button colour.'
        )
...
async def pagify_this(
    ...
    if is_embed:
        pages = list(cf.pagify(big_ass_variable_string, delims=[delim], page_length=page_char))
    else:
        pages = list(cf.pagify(big_ass_variable_string, delims=[delim], page_length=(page_char - 50)))
    ...
    for index, page in enumerate(pages, 1):
        if is_embed:
            embed = discord.Embed(
                ...
                text=page_text.format_map(NoobCoordinate(index=index, pages=len(pages))),
                icon_url=footer_icon
            )
            final_page.append(embed)
        else:
            t = "{page}\n\n{page_text}".format_map(
                    NoobCoordinate(page=page, page_text=page_text, index=index, pages=len(pages))
                )
            final_page.append(t)
    return final_page
